...
int Post4_3out(void){
   ...
   if(Step == GETBASE){
      ...
      if(db == NULL){
         MessageBox("Ошибка открытия БД", "БД 'Пропуска' не открывается.");
      ...
      if(DBFGetErrorNum() != DBF_OK){
         if(DBFGetErrorNum() == DBF_ERROR_RECORD_NOT_FOUND){
            DBFClose(db);
            MessageBox("Выезд по абонементу ЗАПРЕЩЁН!", "Документ недействителен!");
      ...
      switch(CheckNGD(DBFGetFieldAsString(db, "NВЪЕЗДА"), NPost, GetDateType(NULL))){
         ...
         case -2:
            ...
            return 0;
         case 0:
            MessageBox("Выезд по пропуску ЗАПРЕЩЁН!",
      ...
      if(DBFGetFieldAsString(db, nfUseParking)[0] == keyInFree[0]){
         MessageBox("Стоянка", "Выезд по пропуску запрещен!\nНет на стоянке.");
         DBFClose(db);
         Step = ERROROUT;
         return 0;
      }/* if */
      ...
      if(CheckAfterDate(&d, &t, &dt, &tm) && SaveStep != 0){
         MessageBox("Выезд по пропуску",
                    "Предупреждение!\n"
...
*/
   ...
   if(Step == WAITQSL){
      ...
      if(ExtKey == END){
         MessageBox("Выезд по пропуску", "Выезд отменён оператором");
