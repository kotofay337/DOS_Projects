...
int Post4_2(void){
   ...
   if(Step == GETBASE){
      ...
      if(DBFGetErrorNum() != DBF_OK){
         if(DBFGetErrorNum() == DBF_ERROR_RECORD_NOT_FOUND){
            MessageBox("Въезд по стояночному талону ЗАПРЕЩЁН!", "Документ недействителен!");
      ...
      if(CheckAfterDate(&dt, &tm, &d, &t) == TRUE){
         MessageBox("Въезд по стояночному талону", "Въезд еще не разрешен!");
         DBFClose(db);
         Step = ERROROUT;
         return 0;
      }/* if */

      DBFGetFieldAsDate(db, nfsOutPerson[4], &d);
      DBFGetFieldAsTime(db, "ПОВРЕМЯ", &t);
      if(CheckAfterDate(&d, &t, &dt, &tm)){
         MessageBox("Въезд ЗАПРЕЩЁН!",
                    "Документ просрочен!");
         DBFClose(db);
         Step = ERROROUT;
      ...
      if(DBFGetErrorNum() != DBF_OK){
         ...
         if(strcmp(DBFGetFieldAsString(dbParking, nfInUse), keyInUse) == 0){
            MessageBox("Стоянка", "Место занято: %s ,ОСВОБОДИТЕ!", DBFGetFieldAsString(dbParking, nfGRN));
            DBFClose(dbParking);
            DBFClose(db);
            Step = ERROROUT;
            return 0;
   ...
   if(Step == WAITQSL){
      ...
      if(ExtKey == END){
         MessageBox("Въезд по стояночному талону", "Въезд отменён оператором");
