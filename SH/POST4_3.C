   ...
   if(Step == GETBASE){
      ...
      if(DBFGetErrorNum() != DBF_OK){
         if(DBFGetErrorNum() == DBF_ERROR_RECORD_NOT_FOUND){
            DBFClose(db);
            MessageBox("Въезд по абонементу ЗАПРЕЩЁН!", "Документ недействителен!");
      ...
      if(CheckAfterDate(&dt, &tm, &d, &t) == TRUE){
         MessageBox("Въезд по абонементу", "Въезд ещё не разрешен!");
         DBFClose(db);
         Step = ERROROUT;
         return 0;
      }/* if */
      ...
      switch(CheckNGD(DBFGetFieldAsString(db, "NВЪЕЗДА"), NPost, GetDateType(NULL))){
         ...
         case -2:
            ...
            return 0;
         case 0:
            MessageBox("Въезд по абонементу ЗАПРЕЩЁН!",
                       "Номер въезда не совпадает\nни с одним разрешённым\nдля этого абонемента!");
            DBFClose(db);
            Step = ERROROUT;
            return 0;
         case 2:
            MessageBox("Въезд по абонементу", "Разрешается\n"
      ...
      cType = DBFGetFieldAsString(db, nfType)[0];

      if(cType == 'Д' && GetTimeType() == 'Н'){
         MessageBox("Въезд по абонементу ЗАПРЕЩЁН!", "Абонемент ДНЕВНОЙ !!!");
         DBFClose(db);
         Step = ERROROUT;
         return 0;
      }/* if */

      if(cType == 'Н' && GetTimeType() == 'Д'){
         MessageBox("Въезд по абонементу ЗАПРЕЩЁН!", "Абонемент НОЧНОЙ !!!");
         DBFClose(db);
         Step = ERROROUT;
         return 0;
      }/* if */
      ...
      if(NumRunning == 0){
         MessageBox("Въезд по абонементу ЗАПРЕЩЁН!", "Количество въездов исчерпано");
         DBFClose(db);
         Step = ERROROUT;
         return 0;
      }/* if */

      if(DBFGetFieldAsDate(db, nfEndDate, &d)){
         if(NumDays(&dt, &d) < 0){
            MessageBox("Въезд по абонементу ЗАПРЕЩЁН!", "Документ просрочен");
            DBFClose(db);
            Step = ERROROUT;
            return 0;
         }/* if */
      ...
      if(strcmp(DBFGetFieldAsString(db, nfUseParking), keyInUse) == 0){
         MessageBox("Стоянка", "Въезд по абонементу запрещен!\nместо занято");
         DBFClose(db);
         Step = ERROROUT;
         return 0;
      }/* if */
...
*/
   ...
   if(Step == WAITQSL){
      ...
      if(ExtKey == END){
         MessageBox("Въезд по абонементу", "Въезд отменён оператором");
