...
static OBJECT **p = NULL;
      ...
      if(iSHBOnOff == shbsOff){
         return 1;
      }/* if */

      if((Robot == STOP) && (ir1 == irON && ir2 == irON)){
         t = bt;
         Robot = BEGIN;
         t1 = 0.0F;
      }/* if */

      if((Robot == BEGIN) && (ir1 == irOFF || ir2 == irOFF)){
         t = bt;
      }/* if */

      if((Robot == BEGIN) && (ir1 == irON && ir2 == irON)){
         t1 = (bt - t) / CLK_TCK;
...
*/
      ...
      if(s == SHBWAITUP){
         ...
         setcolor_obj_num(p, 100, 10, ir1 == irOFF ? EGA_RED : EGA_LIGHTGREEN, SHOW);
         setcolor_obj_num(p, 100, 11, ir2 == irOFF ? EGA_RED : EGA_LIGHTGREEN, SHOW);
      ...
      if(s == SHBWAITCAR){
         OutTime(p);
         setcolor_obj_num(p, 100, 10, ir1 == irOFF ? EGA_RED : EGA_LIGHTRED, SHOW);
         setcolor_obj_num(p, 100, 11, ir2 == irOFF ? EGA_RED : EGA_LIGHTRED, SHOW);

         if(((bt - t) / CLK_TCK) >= KUSHBTime){
            shbC1 = shbcOK;
         }/* if */
// ----------------
         if((Robot == END) && (ir1 == irOFF && ir2 == irOFF)){
            Robot = STOP;
            s = SHBCOMMANDDOWN;
            mf = 0;
            sound(3180);
