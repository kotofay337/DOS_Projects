...
/* (C) Чумак А.И. 1994 */
/*--------------------------------------------------------------------------*/
short int cmpcolorset_obj_num(OBJECT **obj,unsigned char numobj,unsigned char num,
                        short int color)
{
  short int i,tst=-2;
  short int Ind;
  ...
  if(i != -1){
	...
	switch( *((char*)*(obj+Ind)) ){
	   ...
	   case OBJ_line:
	      if(((struct ln*)*(obj+Ind))->col != color)
		 goto ret;
	      break;
	   case OBJ_rect:
	      if(((struct rc*)*(obj+Ind))->col != color)
		 goto ret;
	      break;
	   case OBJ_bar:
	      if(((struct br*)*(obj+Ind))->col != color)
		 goto ret;
	      break;
	   case OBJ_elip:
	      if(((struct el*)*(obj+Ind))->col != color)
		 goto ret;
	      break;
	   case OBJ_felip:
	      if(((struct fel*)*(obj+Ind))->col != color)
		 goto ret;
	      break;
	   case OBJ_txt:
	      if(((struct tx*)*(obj+Ind))->col != color)
		 goto ret;
	      break;
	   case OBJ_fld:
	      if(((struct fl*)*(obj+Ind))->col != color)
		 goto ret;
	      break;
	   case OBJ_bkgr:
	      if(((struct bk*)*(obj+Ind))->col != color)
		 goto ret;
	      break;
	   case OBJ_pipeline:
