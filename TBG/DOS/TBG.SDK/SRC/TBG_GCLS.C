	  ...
	  for(;(unsigned short int)*((unsigned long*)ptr_Logic+i) == Search;i++){
		...
		break;
		  ...
		  switch( *((char*)*(obj+Ind)) ){
			  ...
			  case OBJ_line:
		 color=((struct ln*)*(obj+Ind))->col;
	   break;
	   case OBJ_rect:
		 color=((struct rc*)*(obj+Ind))->col;
	   break;
	   case OBJ_bar:
		 color=((struct br*)*(obj+Ind))->col;
	   break;
	   case OBJ_elip:
		 color=((struct el*)*(obj+Ind))->col;
	   break;
	   case OBJ_felip:
		 color=((struct fel*)*(obj+Ind))->col;
	   break;
	   case OBJ_txt:
		 color=((struct tx*)*(obj+Ind))->col;
	   break;
	   case OBJ_fld:
		 color=((struct fl*)*(obj+Ind))->col;
	   break;
	   case OBJ_bkgr:
		 color=((struct bk*)*(obj+Ind))->col;
	   break;
	  case OBJ_pipeline:
		 color=((struct pln*)*(obj+Ind))->pl.C2;
	   break;
